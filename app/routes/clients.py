...
def validate_favorites_with_errors(favorites):
    ...
    for prod_id in favorites:
        ...
        if PRODUCTS_SOURCE == "mock":
            exists = prod_id in mem_products
        else:
            resp = requests.get(f"{PRODUCTS_API_URL}/{prod_id}")
...
def create_client(client: ClientCreate, _=Depends(authenticate)):
    ...
    favorites, not_found, duplicates = validate_favorites_with_errors(client.favorites or [])
    error_msgs = []
    if not_found:
...
def update_client(email: str, client_update: ClientUpdate, _=Depends(authenticate)):
    ...
    if client_update.favorites is not None:
        favorites, not_found, duplicates = validate_favorites_with_errors(client_update.favorites)
        error_msgs = []
        if not_found:
            error_msgs.append(f"Produtos inexistentes: {', '.join(not_found)}")
        if duplicates:
